...
async def send_telegram_message(message):
    if not TELEGRAM_ENABLED:
        logging.warning("⚠️ Telegram ist deaktiviert (TELEGRAM_ENABLED=False)")
        return False
    if not BOT_TOKEN or not CHAT_ID:
        logging.error("❌ TELEGRAM_TOKEN oder TELEGRAM_CHAT_ID fehlen.")
        return False
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.post(url, json=payload) as resp:
                ...
                logging.info(f"📬 Telegram-Status: {status}, Antwort: {response}")
                return status == 200
    except Exception as e:
...
async def monitor_loop():
    ...
    if not await send_telegram_message("📡 CoreFlow Watchdog gestartet."):
    ...
    while True:
        try:
            if not is_process_running():
                ...
                if start_coreflow():
                    await send_telegram_message("✅ CoreFlow wurde neu gestartet.")
                else:
                    await send_telegram_message("⚠️ CoreFlow Neustart fehlgeschlagen!")
...
async def shutdown():
    ...
    await send_telegram_message("🛑 CoreFlow Watchdog beendet.")
