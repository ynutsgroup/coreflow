...
logging.basicConfig(
    level=logging.INFO,
    format='%(asctime)s [EnvLoader] %(levelname)s %(message)s'
)

class EnvLoader:
    """Secure, typed, FTMO-compliant environment loader for CoreFlow ecosystem."""

    def __init__(self, env_path: Union[str, Path] = "/opt/coreflow/.env.replay.clean"):
        self.env_path = Path(env_path)
        self.env: Dict[str, Union[str, int, float, bool]] = {}
        self._validation_patterns = {
            'API_KEY': r'^[a-f0-9]{64}$',
            'FTMO_ACCOUNT_ID': r'^\d{9,10}$',
            'REDIS_HOST': r'^[a-zA-Z0-9_.-]+$',
            'REPLAY_FILE': r'^/.*\.csv$',
        }

    def load(self) -> None:
        if not self.env_path.exists():
            log.warning(f".env not found: {self.env_path}")
            return
            ...
            with self.env_path.open("r", encoding="utf-8") as f:
                for line_number, line in enumerate(f, 1):
                    ...
                    if any(token in line for token in [";", "`", "$(", "\\", "[", "]"]):
                        log.warning(f"Ignoring unsafe line {line_number}")
                        continue

                    if "=" not in line:
                        log.warning(f"Invalid format at line {line_number}: {line}")
                        continue
                    ...
                    if key in self._validation_patterns:
                        if not re.match(self._validation_patterns[key], val):
                            log.error(f"Invalid format for {key} at line {line_number}")
                            continue

                    self._set_env(key, val)

        except Exception as e:
            log.exception(f"Error loading .env: {e}")
            raise
    ...
    def validate_ftmo_requirements(self) -> bool:
        ...
        if missing:
            log.error(f"Missing FTMO keys: {missing}")
            return False
        ...
        try:
            ...
            log.error("RISK_PERCENTAGE must be between 0.01 and 5.0")
            return False
    ...
    def get_ai_config(self) -> Dict[str, Union[str, int, float, bool]]:
        return {
            "model_path": self.get("MODEL_PATH", "/opt/coreflow/models/default"),
            "inference_batch": self.get("AI_BATCH_SIZE", 32),
            "confidence_threshold": self.get("CONFIDENCE", 0.8),
            "enable_ml": self.get("AI_ENABLED", True)
        }
